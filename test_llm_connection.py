...
async def test_llm_connection():
    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "Hallo, bist du erreichbar? Antworte nur mit 'Ja'.",
                }
            ],
            max_tokens=10,
        )
    ...
    try:
        ...
        session_state = SessionState(
            ...
            customer=customer,
            messages=[{"role": "user", "content": "Hallo HENK, kannst du mich hören?"}],
        )

        # Initialisiere Agent
        agent = Henk1Agent()
        print("   ✅ HENK1 Agent initialisiert")

        # Verarbeite Anfrage
        print("   🤔 HENK1 verarbeitet Anfrage...")
        decision = await agent.process(session_state)

        print("   ✅ HENK1 hat geantwortet!")
        print()
